...
class Hud {
	// hudNumber: number;
	constructor (public hudNumber: number, public elements: HudElement[]) {
			// this.hudNumber = obj.hudNumber;
	}

	static fromObj(hud: any) {
			let elements = [];
			// console.log(hud);
...
function Ccg_WideX(x: number) {
	return (Ccg_Is43Screen()) ? x : x *cgs.adr43; // * (aspectratio / (4/3))
}

function CG_getComponent(...argv: any) {
	var tmp = argv[4];
	argv[4] = argv[5];
	argv[5] = tmp;
	return argv;
...
var hud0 = {
	...
	cursorhints:	CG_getComponent(.5 * SCREEN_WIDTH - .5 * 48, 260, 48, 48, qtrue, STYLE_NORMAL), // FIXME: widescreen ?
...
declare var canvas: fabric.Canvas;
var info = "test";
